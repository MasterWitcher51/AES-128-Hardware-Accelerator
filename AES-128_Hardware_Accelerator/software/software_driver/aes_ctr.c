#include "aes_ctr.h"
#include <string.h>

static const uint8_t S_BOX[256] = {
    
};

static const uint8_t RCON[10] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};

// Main Steps

void sub_bytes(uint8_t state[4][4]) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            state[i][j] = S_BOX[state[i][j]];
        }
    }
}

void shift_rows(uint8_t state[4][4]) {
    uint8_t temp;
    // Row 1
    temp = state[1][0];
    state[1][0] = state[1][1]; state[1][1] = state[1][2]; state[1][2] = state[1][3]; state[1][3] = temp;
    // Row 2
    temp = state[2][0]; uint8_t temp2 = state[2][1];
    state[2][0] = state[2][2]; state[2][1] = state[2][3]; state[2][2] = temp; state[2][3] = temp2;
    // Row 3
    temp = state[3][3];
    state[3][3] = state[3][2]; state[3][2] = state[3][1]; state[3][1] = state[3][0]; state[3][0] = temp;
}

uint8_t xtime(uint8_t a) {
    return (a & 0x80) ? (((a << 1) ^ 0x1B) & 0xFF) : (a << 1);
}

void mix_single_column(uint8_t a[4]) {
    uint8_t t = a[0] ^ a[1] ^ a[2] ^ a[3];
    uint8_t u = a[0];
    a[0] ^= t ^ xtime(a[0] ^ a[1]);
    a[1] ^= t ^ xtime(a[1] ^ a[2]);
    a[2] ^= t ^ xtime(a[2] ^ a[3]);
    a[3] ^= t ^ xtime(a[3] ^ u);
}

void mix_all_columns(uint8_t state[4][4]) {
    for (int j = 0; j < 4; j++) {
        uint8_t column[4] = {state[0][j], state[1][j], state[2][j], state[3][j]};
        mix_single_column(column);
        for (int i = 0; i < 4; i++) state[i][j] = column[i];
    }
}

void add_round_key(uint8_t state[4][4], uint8_t k[4][4]) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            state[i][j] ^= k[i][j];
        }
    }
}

// Key Schedule Functions
void rot_word(uint8_t word[4]) {
    uint8_t temp = word[0];
    word[0] = word[1]; word[1] = word[2]; word[2] = word[3]; word[3] = temp;
}

void sub_word(uint8_t word[4]) {
    for (int i = 0; i < 4; i++) word[i] = S_BOX[word[i]];
}

void key_expansion(const uint8_t key_bytes[16], uint8_t round_keys[11][4][4]) {
    uint8_t w[44][4];
    for (int i = 0; i < 4; i++) memcpy(w[i], &key_bytes[i * 4], 4);

    for (int i = 4; i < 44; i++) {
        uint8_t temp[4];
        memcpy(temp, w[i - 1], 4);
        if (i % 4 == 0) {
            rot_word(temp);
            sub_word(temp);
            temp[0] ^= RCON[(i / 4) - 1];
        }
        for (int j = 0; j < 4; j++) w[i][j] = w[i - 4][j] ^ temp[j];
    }

    // Group into round keys matrix
    for (int i = 0; i < 11; i++) {
        for (int r = 0; r < 4; r++) {
            for (int c = 0; c < 4; c++) {
                round_keys[i][r][c] = w[i * 4 + c][r];
            }
        }
    }
}

void aes_encrypt(const uint8_t plaintext_bytes[16], const uint8_t key_bytes[16], uint8_t ciphertext[16]) {
    uint8_t state[4][4];
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            state[r][c] = plaintext_bytes[r + 4 * c];
        }
    }

    // Key Expansion to generate round keys
    uint8_t round_keys[11][4][4];
    key_expansion(key_bytes, round_keys);

    // Initial Round
    add_round_key(state, round_keys[0]);

    // 9 Rounds
    for (int r = 1; r < 10; r++) {
        sub_bytes(state);
        shift_rows(state);
        mix_all_columns(state);
        add_round_key(state, round_keys[r]);
    }

    // Final Round
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, round_keys[10]);

    // Flatten to list
    for (int c = 0; c < 4; c++) {
        for (int r = 0; r < 4; r++) {
            ciphertext[r + 4 * c] = state[r][c];
        }
    }
}

void aes_ctr_xcrypt(uint8_t *data, size_t len, const uint8_t key[16], uint8_t nonce[16]) {
    uint8_t counter[16];
    memcpy(counter, nonce, 16);
    uint8_t keystream[16];

    for (size_t i = 0; i < len; i += 16) {
        aes_encrypt(counter, key, keystream);
        
        size_t chunk = (len - i < 16) ? (len - i) : 16;
        for (size_t j = 0; j < chunk; j++) {
            data[i + j] ^= keystream[j];
        }

        // Increment counter
        for (int k = 15; k >= 0; k--) {
            if (++counter[k] != 0) break;
        }
    }
}